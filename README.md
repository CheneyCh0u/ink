...
- 外观、启动侧边栏、默认窗口大小、字体、光标、Option-as-Meta、选中即复制和 scrollback 容量可配置。
- 配置文件变更会自动重载，无需重启应用。
...
也可以从侧边栏底部的“设置”或 <kbd>⌘ ,</kbd> 打开内嵌设置页。设置页写回
...
`appearance`、字体、光标和交互设置会立即应用；`sidebar.startup_mode` 与默认窗口
尺寸在下次启动时生效，`scrollback.lines` 只影响新会话。
