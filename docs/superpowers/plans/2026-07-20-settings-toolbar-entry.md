...
- `Sources/InkShell/SettingsViewController.swift`：删除会暗示需要提交改动的“完成”按钮。
...
- Produces: 共同顶部栏视图层级；设置中点击标签、新建标签或选中齿轮先返回终端；
  设置页不显示“完成”按钮。
...
删除 `sidebarVC.onSettings` 接线，新增切换逻辑：
...
在 `SettingsViewController.makeHeader()` 中删除标题为“完成”的 `NSButton` 及其尾部约束，
标题保留原 leading 与垂直约束。保留 `onDone` 和 Escape 路径作为键盘返回行为。
...
标签、齿轮 hover 和选中态、当前与其它标签返回、齿轮再次点击返回、加号返回、新建
项目、`⌘,` 与 Escape。窗口不得缩放或跳动，VoiceOver 读出“设置”，设置页不得显示
“完成”按钮。
