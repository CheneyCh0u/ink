...
menu、以真实 `MainWindowController` 打开 key window 并进入 modal session 后，断言
`NSApp.modalWindow === window`；再调用 submenu `update()`，三个字号项和侧边栏项的实际 target
均为该窗口控制器，且侧边栏项 enabled。fixture 会恢复 main menu、关闭窗口并删除临时目录；不会再由
`NSWindow` 截获旧 selector。
