...
**Interfaces:**
- Consumes: Task 2 的三个 `MainWindowController` ObjC actions 与专属的 `MainWindowController.toggleSidebarMode(_:)`。
...
struct FontSizeMenuTests {
    ...
    func mainWindowResponderChainEnablesSidebarMenu() throws {
        ...
        let view = try #require(menu.items.first { $0.submenu?.title == "显示" }?.submenu)
        view.update()
        let items = try ["放大字号", "缩小字号", "恢复默认字号", "切换侧边栏"].map { title in
        ...
        #expect(targets.allSatisfy { ($0 as? MainWindowController) === fixture.controller })
...
Expected: FAIL，找不到“放大字号”等菜单项，且“切换侧边栏”仍为 `⌘0`；若恢复通用
`NSSplitViewController.toggleSidebar(_:)` selector，运行时会先被 `NSWindow` 截获，导致侧边栏项
无法解析到 `MainWindowController` 且被禁用。
...
用显式 `NSMenuItem` 替换现有侧边栏项，以便设置修饰键并通过专属 selector 避开 responder chain
中的 `NSWindow.toggleSidebar(_:)`：
...
在 `MainWindowController` 中添加 `@objc func toggleSidebarMode(_ sender: Any?)` 这个极薄适配器：
它只调用既有的无参三态切换逻辑，不能恢复通用 `toggleSidebar(_:)` selector。
...
Expected: PASS；四个菜单项的 selector、key equivalent 和 modifier mask 全部匹配。安装完整 main
menu 并以真实 `MainWindowController` 调用 submenu `update()` 后，三个字号项和侧边栏项的实际
target 均为该窗口控制器，且侧边栏项 enabled；不会再由 `NSWindow` 截获旧 selector。
