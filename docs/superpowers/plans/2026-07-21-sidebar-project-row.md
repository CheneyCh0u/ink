...
\`mouseEntered\` 调用 \`setCloseButtonRevealed(true)\`，\`mouseExited\` 调用 \`setCloseButtonRevealed(false)\`；保留 \`hoveredRowTitle\` 以跨 reload 恢复关闭图标本身。

- [ ] **Step 6: 运行侧边栏测试确认 GREEN**

Run: \`swift test --filter ProjectSidebarLayoutTests\`

Expected: 布局 suite 全部 PASS；长路径测试证明三个文本 frame 跨 hover、reload、exit 不变。

- [ ] **Step 7: 运行 Shell 测试并提交布局修复**
