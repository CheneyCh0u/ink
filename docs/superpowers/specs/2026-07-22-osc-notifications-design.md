...
### 方案 B：Parser 发送词法生命周期，Terminal 有界累积并负责语义（采用）

Parser 不保存 OSC 载荷，只向具体化的 `Terminal` 发送开始、数据、忽略控制字节、完成
和取消动作。Terminal 的旁路累积器让普通 OSC 最多保存 4096 字节，同时允许 OSC 52
流式解码更大的 Base64 输入；收到 BEL/ST 后再由 `Terminal.oscDispatch` 识别 9/777、
严格解码 UTF-8 并构造值事件。这个方案保持现有分层，复用标题、超链接和 OSC 133 的
分发入口，也避免 Parser 为 OSC 52 常驻约 1.4 MiB 编码缓冲。
...
- ESC 后紧接 `]` 时，新 OSC 覆盖未完成序列，旧累积器立即释放并重新开始。
...
Terminal 使用独立状态标签和原地 `ContiguousArray<UInt8>` 字段。逐字节追加只发生在
唯一持有的值缓冲上，不创建每字节数组或切片，也不通过枚举关联值取出、写回大数组。
...
- 通用 OSC 缓冲有 4096 字节硬上限；结束或取消后释放容量。普通输出、grid 与渲染
  热路径不新增分配。

### 控制字符

为保持既有 OSC 0/2、8、133 和 OSC 52 兼容语义，Parser 仍忽略 BEL/ESC 之外的 C0，
但会把“本序列出现过 C0”作为词法信号交给 Terminal。通用协议继续使用剥离后的载荷；
OSC 9/777 看到该信号则整段失效。Terminal 语义验证还拒绝 U+007F 和
U+0080...U+009F，覆盖 UTF-8 编码的 DEL/C1。这样既不改变旧协议，也不会把控制内容
带进系统通知 UI。
