...
2026-07-22 在 Apple M4 Pro、macOS 27.0（26A5388g）上运行
`swift test -c release --filter WorkspaceSnapshotTests`。上限用例包含 4 个项目、
每项目 32 个单 pane 标签，共 128 个 pane；这是损坏快照恢复的总资源预算，不是
建议用户日常同时打开的会话数量。

| 操作 | 实测 | 单次平均 |
|---|---:|---:|
| schema 1 JSON 大小 | **16,543 bytes** | — |
| 编码 + 解码 + 语义校验 × 100 | **64.249 ms** | **0.642 ms** |
| 隔离 UserDefaults save + load × 100 | **83.520 ms** | **0.835 ms** |

原始输出位于 `/tmp/ink-issue68-workspace-perf.txt`。快照远低于测试约束的 1 MiB；
实际交互状态通常只有少量项目、标签与 pane，会更小。结构变化采用 250 ms 单次
合并任务，正常关闭则在终止 PTY 和清空标签前同步保存，因而拖动比例、连续切换和
重命名不会逐事件写盘，也不会在退出时丢失最后状态。

这组数字只覆盖 Codable、语义校验和 UserDefaults 单值读写，不包含恢复时启动新
shell 的成本。恢复会为每个合法 pane 创建新的 PTY，最多 128 个；只有活动标签创建
Metal 视图。工作目录查询与快照捕获只发生在结构变化和关闭时，不接入 PTY 输出、
Parser、grid、scrollback 或渲染循环，因此本项不需要热路径 Time Profiler 采样。
