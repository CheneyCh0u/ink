...
82,000,000 字节，耗时 1.261226375 秒、吞吐 62.0 MB/s、footprint 增量
12.8 MB；每 1,000 行插入一段 OSC 8 的稀疏场景共 82,041,000 字节，耗时
1.345768875 秒、吞吐 58.1 MB/s、footprint 增量 13.6 MB。原始输出分别位于
`/tmp/ink-links-issue66-plain.txt` 和 `/tmp/ink-links-issue66-sparse.txt`。

普通输出仍不创建链接旁路表；只有实际出现 OSC 8 时才保存目标和范围。稀疏
场景相对普通文本慢约 6.3%，1000 个带链接逻辑行增加约 0.8 MB footprint。
