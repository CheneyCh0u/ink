# ink 设计系统

## 视觉方向

选定方案为 **A「雾白」**：Codex 式的克制信息架构，配合更明显的 macOS
半透明侧边栏。终端内容区保持安静、清晰，侧边栏承担材质和品牌识别。

浅色模式使用暖雾玻璃与接近白纸的终端；深色模式使用暖石墨玻璃与低眩光
墨黑终端。深色不是浅色反相，而是重新分配表面亮度层级。

## 权威实现

原生应用的全局 token 定义在 `Sources/InkDesign/`（独立 SwiftPM 模块，
外壳与渲染层都依赖它，`TerminalCore` 不依赖）：

- `InkDesignTokens.swift` — 外壳语义色、间距、圆角、字体、动效
- `InkTerminalPalette.swift` — 终端 ANSI 16 色与前景 / 背景 / 光标 / 选区

业务组件只能使用语义 token，不得直接写 RGB、圆角或间距数值。

颜色由动态 `NSColor` 提供，会随 macOS 的 effective appearance 实时解析。
SwiftUI 使用时通过 `Color(nsColor:)` 桥接，不建立第二套颜色表。

侧边栏使用：

- `NSVisualEffectView.Material.sidebar`
- `NSVisualEffectView.BlendingMode.behindWindow`
- `NSVisualEffectView.State.active`

材质由侧栏根视图自己承载，并放在普通的可折叠 `NSSplitViewItem` 中。不要使用
`sidebarWithViewController` 或把 split item 标记成系统 sidebar；较新的 macOS
会把这类侧栏自动处理成带圆角和外边距的浮动面板，破坏从窗口顶部贯穿到底部的
一体壳层。

侧边栏有三种显示状态，顶部开关按以下顺序循环：

1. `expanded`：258pt，显示项目路径、备注/会话数与完整操作。
2. `compact`：72pt，只保留 40×40pt 项目图标轨道与底部新建图标；此宽度
   完整覆盖 macOS 红绿灯的横向占位。
3. `hidden`：完全隐藏，把空间交还终端；再次点击回到 `expanded`。

图标态不是展开态的压缩截图，而是独立的信息层级：文字全部移除，项目路径通过
tooltip 和辅助功能标签提供。活动项目继续使用 `selected` 高光。

项目可在右键菜单选择 Finder 式红、橙、黄、绿、蓝、紫、灰七种标记或清除标记。
展开态在项目行左侧固定预留标记列：已标色项目显示 8pt 圆点，未标色项目留空，
文件夹图标与文字始终对齐。图标态在项目卡片左缘内侧显示 4×26pt 圆角短色条。
颜色只用于快速识别项目，不改变文件夹图标颜色，也不覆盖活动项目背景。标记颜色
必须通过 `InkDesignTokens.ProjectLabel` 获取，不能在侧边栏组件内写 RGB。

不要用手工透明色或模糊截图模拟系统材质。`sidebarFallback` 只用于材质不可用、
Reduce Transparency 开启或测试快照。

## 色彩角色

| Token | 浅色 | 深色 | 用途 |
|---|---:|---:|---|
| `canvas` | `#F9F9F7` | `#171819` | 窗口底层 |
| `terminal` | `#FDFDFD` | `#111314` | 终端内容区 |
| `sidebarFallback` | `#EFEAE2` / 78% | `#262421` / 86% | 侧栏材质降级色 |
| `elevated` | 白 / 88% | 白 / 5% | 浮层与提升态 |
| `selected` | 白 / 72% | 白 / 8.5% | 侧边栏选中行（白色高光浮在材质上；暖褐色在真机叠壁纸透光会发闷，弃用） |
| `pill` | 黑 / 5.5% | 白 / 7.5% | 标签 pill 活动态 |
| `textPrimary` | `#303238` | `#E2E3DF` | 主文字 |
| `textSecondary` | `#777A80` | `#92969C` | 次级文字 |
| `separator` | `#70737A` / 22% | 白 / 8% | 发丝分隔线 |
| `accent` | `#168FAF` | `#58B6C9` | 品牌、焦点、光标 |
| `success` | `#228F5B` | `#67C88D` | 成功状态 |
| `warning` | `#B67F2B` | `#D9B25E` | 警告状态 |
| `branch` | `#9B67C8` | `#C29ADA` | Git 分支语义 |
| `danger` | `#D84A4A` | `#F06B68` | 错误与危险操作 |

## 终端 ANSI 调色板

内置五个配色家族，每个家族都有独立设计的浅色与深色快照，不做机械反相。
设置只保存家族名，当前界面外观决定实际使用哪个快照。

| 家族 | 浅色前景 / 背景 | 深色前景 / 背景 | 主强调 |
|---|---|---|---|
| 暖墨 `warm` | `#3D3929` / `#F5EEDC` | `#E8DFCC` / `#17140F` | 珊瑚 |
| 石墨蓝 `graphite` | `#303844` / `#F7F9FC` | `#DCE3EC` / `#101419` | 冷蓝 |
| 松针 `pine` | `#263A34` / `#F5F8F6` | `#D9E8DF` / `#101713` | 青绿 |
| 紫墨 `plum` | `#3C3443` / `#FAF7FA` | `#E8DDE9` / `#171218` | 梅紫 |
| 中性炭 `neutral` | `#34363A` / `#FDFDFD` | `#E2E3DF` / `#111314` | 中性蓝 |

每个快照都定义 ANSI 0–15、默认前景与背景、光标、选区和搜索高亮。完整色值以
`InkTerminalPalette.swift` 为准；设置页样例与真实终端直接消费同一份快照。

**热路径纪律**：渲染器只消费 `InkTerminalPalette` 的值快照（打包 sRGB），
主题或外观切换时整体换快照重传 uniform，不重建 glyph atlas。帧循环内禁止出现
`NSColor` 动态解色。
256 色索引表的 16–255 段（6×6×6 色立方 + 灰阶）按 xterm 标准公式在渲染器内
生成，不属于主题；真彩色 SGR 直接透传。

## 尺度

- 间距：`4 / 8 / 12 / 16 / 24 / 32`
- 圆角：控件 `6`、列表项 `10`、面板 `14`、窗口 `22`
- 侧边栏宽度：展开 `258`、图标轨道 `72`
- 点击反馈：`140ms`
- 状态变化：`180ms`

## 顶部标签栏

标签最小宽度为 112pt，理想宽度为 168pt，最大宽度为 240pt。短标题使用理想
宽度，长标题最多扩展到最大宽度；空间不足时可压缩到最小宽度，但不得继续压扁。
标签之间的间距为 6pt；进入溢出模式时，标签区同样为 28pt 溢出按钮预留 6pt
间距。新建标签等外壳控件沿用既有系统布局间距，贴标签栏右缘。

所有标签以最小宽度仍放不下时，顶部栏只显示包含活动标签的连续区间，并在新建标签
按钮左侧显示 28pt 的“更多标签”原生菜单。活动标签始终可见；区间只做包含活动标签
所需的最小移动。菜单只列隐藏标签，保持原始顺序和原始索引。

窗口连续缩放时，按当前可用宽度重新计算连续可见区间：活动标签始终可见，区间只作
包含活动项所需的最小移动；变宽时按同一规则恢复更多可见项。

标签栏剩余空间继续作为窗口拖动区域。标签、溢出和新建按钮不触发窗口拖动。
溢出入口使用 SF Symbol、tooltip 和“更多标签”辅助功能标签，不自绘菜单，不增加横向
滚动位置，也不进入终端逐帧渲染路径。

外壳字体使用系统字体。终端默认使用 Maple Mono NF CN 15pt；系统缺少该字体或
`[font] family` 配置无效时，回退到系统等宽字体。默认字体度量为
`line_height = 1.0`；`adjust_cell_height = 1` 表示增加 1 个物理像素。字形通过
`thicken = true` 和 `thicken_strength = 128` 增粗。

Ink 不打包 Maple Mono NF CN。字体由用户系统提供，避免增加应用体积和常驻内存；
若将来需要内嵌字体，仍须按依赖纪律先讨论。

> 备考：Ghostty 把 JetBrains Mono 编译进二进制（系统字体目录查不到），
> 对比字体渲染时注意参照应用的字体来源。若将来要内嵌字体，JetBrains
> Mono 为 OFL 协议可打包，约 +5MB，需按依赖纪律先讨论。

## 使用约束

- 不在 `TerminalCore` 引入本文件或 AppKit。
- 侧边栏负责材质，终端内容区使用不透明背景，保证字符对比度和渲染稳定。
- 深色正文不使用纯白，避免长时间使用产生眩光。
- 只在交互元素使用强调色，强调色不铺满大面积背景。
- 外观切换不重建业务状态；视图从动态颜色重新解析即可。
- 遵守 Reduce Motion 与 Reduce Transparency 系统辅助功能设置。

## 内嵌设置页

设置页使用主内容区替换终端工作区，不创建独立窗口。顶部标签栏在终端与设置状态中
始终保留。全局设置入口是侧边栏底部的齿轮：底部操作行由左侧加号（新建项目）和
右侧齿轮（设置）两枚 28pt 方形纯图标按钮组成，不显示文字，名称放在 tooltip 与
辅助功能标签里（“新建项目”“设置（⌘,）”）。按钮默认无背景，hover 显示浅色圆角
底；设置打开时齿轮显示选中背景，点击任意标签或再次点击选中齿轮返回终端。设置项
即时应用，不显示“完成”或“保存”按钮。展开态按钮距侧栏两缘 `Spacing.xs`；图标态
侧栏只有 72pt 宽，收紧为 `Spacing.xxs` 以保留两枚按钮之间的间隙。侧边栏完全隐藏
时用 `⌘,` 或菜单栏进入设置。

设置内容沿用 A「雾白」的表面层级：窗口 canvas 承载内容，逻辑分组使用
`elevated`，不新增装饰性渐变或阴影。分组行使用系统输入控件，终端区域提供配色
选择和实时示例。样例同时展示默认字色、路径、成功、警告、错误和分支等 ANSI
语义色，并使用所选主题的当前明暗背景保证对比度可直接判断。项目选择与 Esc 都
返回终端。

`~/.config/ink/config.toml` 是唯一配置来源。设置页只替换已知键，必须保留用户
手写注释、空行和未知字段。外观与终端参数立即生效；启动侧边栏和默认窗口尺寸
下次启动生效；scrollback 容量只对新会话生效。

iCloud 使用独立分组，固定放在“交互”和“高级”之间。第一行是“自动上传配置”系统
开关，第二行左侧显示文字状态，右侧并排“上传到云端”和“拉取云端配置”两个次级
按钮。关闭自动上传后两个手动按钮仍可用；进行中同时禁用开关和按钮。成功、失败、
不可用和云端为空都用文字表达，颜色不能作为唯一状态提示。覆盖不同配置时使用 sheet，
默认焦点为取消，确认按钮必须写明上传或拉取方向。

## 应用图标

当前应用图标为 **A「雾白墨滴」**：暖雾白底板保证浅色、深色桌面都有稳定
轮廓，深色墨滴对应产品名 `ink`，内部的 `>_` 对应终端。

- SVG 母版：`Assets/AppIcon.svg`
- 标准尺寸：`Assets/AppIcon.iconset/`
- SwiftPM / 打包资源：`Sources/ink/Resources/AppIcon.icns`

正式生成 `.app` 时，将 `AppIcon.icns` 复制到 `Contents/Resources/`，并在
`Info.plist` 设置 `CFBundleIconFile` 为 `AppIcon`。当前 SwiftPM 可执行程序也会
从资源 bundle 加载同一份图标并设置为 Dock 图标。
