...
展开态不显示标记，图标态在项目卡片左缘内侧显示 4×26pt 圆角短色条。颜色只用于
快速切换时识别项目，不改变文件夹图标颜色，也不覆盖活动项目背景。标记颜色必须通过
...
设置页使用主内容区替换终端工作区，不创建独立窗口。展开侧边栏在底部显示
“新建项目”和“设置”，中间用发丝线分隔；图标态保留加号与齿轮两个纵向入口；
侧边栏完全隐藏时仍可用 `⌘,` 进入。

设置内容沿用 A「雾白」的表面层级：窗口 canvas 承载内容，逻辑分组使用
`elevated`，不新增装饰性渐变或阴影。分组行使用系统输入控件，终端字体区域提供
实时示例。项目选择、Esc 与“完成”都返回终端。
